...
import random
import logging
from bertmodel.similarity import BertSim
...
class Model():
    ...
    def _init_train(self):
        ...
        with self.train_graph.as_default():
            ...
            self.train_in_seq_mask = tf.placeholder(tf.int32, shape=[self.batch_size, None])
            self.train_in_seq_segment = tf.placeholder(tf.int32, shape=[self.batch_size, None])
            ...
            self.train_target_seq_mask = tf.placeholder(tf.int32, shape=[self.batch_size, None])
            self.train_target_seq_segment = tf.placeholder(tf.int32, shape=[self.batch_size, None])

            self.train_target_seq_len = tf.placeholder(tf.int32, shape=[self.batch_size])

            output = seq2seq.seq2seq(self.train_in_seq, self.train_in_seq_len,self.train_in_seq_mask,self.train_in_seq_segment,
                    self.train_target_seq, self.train_target_seq_len,self.train_target_seq_mask,self.train_target_seq_segment,
                    len(self.train_reader.vocabs),
                    self.num_units, self.layers, self.dropout,self.max_len,self.bert_model,self.init_train)
            print(output)
            self.output=output
            ...
            self.loss,self._cost = seq2seq.seq_loss(output, self.train_target_seq,
                    self.train_target_seq_len)
    ...
    def _init_eval(self):
        ...
        with self.eval_graph.as_default():
            ...
            self.eval_in_seq_mask = tf.placeholder(tf.int32, shape=[self.batch_size, None])
            self.eval_in_seq_segment = tf.placeholder(tf.int32, shape=[self.batch_size, None])
            self.eval_in_seq_len = tf.placeholder(tf.int32, shape=[self.batch_size])
            self.eval_output = seq2seq.seq2seq(self.eval_in_seq,
                    self.eval_in_seq_len,self.eval_in_seq_mask, self.eval_in_seq_segment,
                    None, None,None, None,
            ...
            for step in range(0, self.eval_reader.data_size):
                ...
                outputs = self.eval_session.run(
                        ...
                        feed_dict={
                            ...
                            self.eval_in_seq_mask: in_seq_mask,
                            self.eval_in_seq_segment: in_seq_segment})
